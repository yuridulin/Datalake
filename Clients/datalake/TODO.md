...
**Зачем нужно:**
- Защита от падений компонентов
- Показ понятных сообщений об ошибках пользователю
- Предотвращение полного краха приложения

**Что нужно сделать:**

#### Создать глобальный Error Boundary
- Файл: `src/app/components/ErrorBoundary.tsx`
- Реализовать класс-компонент `ErrorBoundary` с методами:
  - `componentDidCatch(error, errorInfo)`
  - `getDerivedStateFromError(error)`
- Показывать понятное сообщение об ошибке
- Добавить кнопку "Перезагрузить страницу" или "Вернуться назад"
- Логировать ошибки в систему логирования

#### Интегрировать Error Boundary
- Обернуть `<AppLayout />` в `ErrorBoundary` в `appRouter.tsx`
- Добавить Error Boundary для критичных секций (например, формы)

#### Исправить AppError.tsx
- Текущий компонент использует `Alert.ErrorBoundary` неправильно
- Заменить на правильную обработку ошибок роутера или интегрировать с Error Boundary

### 1.2. Общий подход к логированию ошибок

**Зачем нужно:**
- Централизованное логирование
- Убрать `console.log/error` из продакшена
- Возможность отправки ошибок на сервер для мониторинга

**Что нужно сделать:**

#### Создать сервис логирования
- Файл: `src/services/logger.ts`
- Уровни логирования: `debug`, `info`, `warn`, `error`
- В dev-режиме использовать `console.*`
- В prod-режиме отправлять только `error` и `warn` на сервер (опционально)
- Методы:
  - `logger.debug(...args)`
  - `logger.info(...args)`
  - `logger.warn(...args)`
...
- Найти все использования `console.log/error/warn` (79 вхождений)
- Заменить на соответствующие методы logger
- Добавить контекст для ошибок (компонент, действие)

#### Интегрировать в stores
- В `BaseCacheStore` и всех stores использовать logger вместо `console.error`
- Добавить контекст ошибок (название store, метод, параметры)

#### Интегрировать в Error Boundary
- Error Boundary должен логировать ошибки через logger
