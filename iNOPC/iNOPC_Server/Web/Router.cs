...
namespace iNOPC.Server.Web
{
	public class Router
	{
				...
				try
				{
					...
					if (Defence.License == LicenseMode.ExpiredTrial)
					{
						res = new ResponseModel
						{
							...
							String = JsonConvert.SerializeObject(new { Error = "Триал-период завершён" })
						};
					}
					else
					{
		...
		ResponseModel Action(string methodName, string body, WindowsPrincipal user)
		{
			...
			var controller = Type.GetType("iNOPC.Server.Web.Api." + methodParts[0] + "Controller", false, true);
			if (controller == null)
			{
				return new ResponseModel
				{
			...
			instance.AccessedType = AccessType;
			instance.User = user;
			...
			if (methodParams.Length > 0)
			{
				...
				var param = JsonConvert.DeserializeObject(body, methodParams[0].ParameterType);

				result = action.Invoke(instance, new object[] { param });
			}
			else
			{
				result = action.Invoke(instance, null);
		...
		ResponseModel Document(string filename)
		{
			Console.WriteLine($"[HTTP DOC] {filename}");

			string basePath = AppDomain.CurrentDomain.BaseDirectory + "\\webConsole\\";
