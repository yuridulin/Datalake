...
export default function BlockForm() {
	...
	const [ propModal, setPropModal ] = useState({ open: false, old: '', key: '', value: '' })
	const [ tagModal, setTagModal ] = useState({ open: false, index: 0, type: 0, tagId: 0, name: '' })
	...
	const RelType = (type: number) => {
		switch (type) {
			case 0: return "Входные"
			case 1: return "Выходные"
			case 2: return "Связанные"
			default: return "Неизвестный вид отношений"
		}
	}
	...
	const Tag = (tagId: number) => {
		try {
			...
			return <NavLink to={'/tags/' + tagId}><Button>{tag.label}</Button></NavLink>
		}
		catch (e) {
			return 'не найден'
		}
		...
		: loading
			...
			: <>
				...
				<FormRow title="Описание">
					<Input.TextArea value={block.Description} onChange={e => setBlock({ ...block, Description: e.target.value })} />
				</FormRow>
				<FormRow title="Родительский блок">
					<Button onClick={() => router.navigate(block.ParentId === 0 ? '/blocks' : '/blocks/' + block.ParentId)}>Перейти</Button>
				</FormRow>
				<FormRow title="Вложенные блоки">
					{block.Children && block.Children.map(b => 
						<div>
							<Button onClick={() => router.navigate(b.Id === 0 ? '/blocks' : '/blocks/' + b.Id)}>{b.Name}</Button>
						</div>
					)}
					<Button onClick={create}>Создать</Button>
				</FormRow>
				<FormRow title="Свойства">
					{block.Properties && Object.keys(block.Properties).map(prop => 
						<div
							key={prop}
							className="table-row"
							style={{ display: 'grid', gridTemplateColumns: '1fr 2fr' }}
							onClick={() => setPropModal({ open: true, old: prop, key: prop, value: block.Properties[prop] })}
						>
							<span>{prop}</span>
							<span>{block.Properties[prop]}</span>
						</div>
					)}
					<Button onClick={() => setPropModal({ open: true, old: '', key: '', value: '' })}>Добавить свойство</Button>
				</FormRow>
				<FormRow title="Теги">
					<div className="table">
						...
						{block.Tags && block.Tags.map((rel, i) =>
							<div
								key={i}
								className="table-row"
								onClick={() => setTagModal({
					...
					<div>
						<Button onClick={() => setTagModal({ open: true, index: -1, tagId: 0, type: 0, name: '' })}>Добавить тег</Button>
					</div>
				</FormRow>
				<Modal
					title={propModal.old}
					...
					footer={[
						...
						<Button onClick={setProperty} type="primary">Сохранить</Button>
					]}
				>
					<FormRow title="Свойство">
						<Input value={propModal.key} onChange={e => setPropModal({ ...propModal, key: e.target.value })} />
					</FormRow>
					<FormRow title="Значение">
						<Input value={propModal.value} onChange={e => setPropModal({ ...propModal, value: e.target.value })} />
					</FormRow>
				</Modal>
				<Modal
					title={tagModal.index < 0 ? 'Новый связанный тег' : 'Связанный тег #' + tagModal.index}
					open={tagModal.open}
					onCancel={() => setTagModal({ ...tagModal, open: false })}
					footer={[
						<Button onClick={delTag}>Удалить</Button>,
						<Button onClick={setTag} type="primary">Сохранить</Button>
					]}
				>
