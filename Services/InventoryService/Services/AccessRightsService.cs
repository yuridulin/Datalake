	...
	ILogger<AccessRightsService> logger)
{
	...
	{
		logger.LogDebug("Событие изменения прав доступа");

		user.ThrowIfNoGlobalAccess(AccessType.Admin);

		DatabaseResult<AccessRights> result;
		...
		if (request.UserGroupGuid.HasValue)
			result = await accessRightsRepository.ReplaceUserGroupRightsAsync(
				...
				request.Rights.Select(x => new AccessRights(null, request.UserGroupGuid, false, x.TagId, x.SourceId, x.BlockId, x.AccessType)));

		else if (request.UserGuid.HasValue)
			result = await accessRightsRepository.ReplaceUserRightsAsync(
				request.UserGuid.Value,
				request.Rights.Select(x => new AccessRights(request.UserGuid, null, false, x.TagId, x.SourceId, x.BlockId, x.AccessType)));

		else if (request.SourceId.HasValue)
			result = await accessRightsRepository.ReplaceSourceRightsAsync(
				request.SourceId.Value,
				request.Rights.Select(x => new AccessRights(request.UserGuid, request.UserGroupGuid, false, null, request.SourceId, null, x.AccessType)));

		else if (request.BlockId.HasValue)
			result = await accessRightsRepository.ReplaceBlockRightsAsync(
				request.BlockId.Value,
				request.Rights.Select(x => new AccessRights(request.UserGuid, request.UserGroupGuid, false, null, null, request.BlockId, x.AccessType)));

		else if (request.TagId.HasValue)
			result = await accessRightsRepository.ReplaceTagRightsAsync(
				request.TagId.Value,
				request.Rights.Select(x => new AccessRights(request.UserGuid, request.UserGroupGuid, false, request.TagId, null, null, x.AccessType)));
