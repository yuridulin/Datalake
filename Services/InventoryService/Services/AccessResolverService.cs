...
namespace Datalake.InventoryService.Services;

/// <summary>
/// Функции предварительного расчета прав доступа для пользователей
/// </summary>
public class AccessResolverService
{
	/// <summary>
	...
	public DatalakeAccessState Resolve(DatalakeDataState state)
	{
		// Оптимизация: ленивая инициализация словарей правил
		var userGlobalRules = new Dictionary<Guid, AccessRule>();
		var groupGlobalRules = new Dictionary<Guid, AccessRule>();

		var userRulesToSources = new Dictionary<Guid, Dictionary<int, AccessRule>>();
		var userRulesToBlocks = new Dictionary<Guid, Dictionary<int, AccessRule>>();
		var userRulesToTags = new Dictionary<Guid, Dictionary<int, AccessRule>>();

		var groupRulesToSources = new Dictionary<Guid, Dictionary<int, AccessRule>>();
		var groupRulesToBlocks = new Dictionary<Guid, Dictionary<int, AccessRule>>();
		var groupRulesToTags = new Dictionary<Guid, Dictionary<int, AccessRule>>();

		foreach (var r in state.AccessRights)
		{
			var rule = new AccessRule(r.Id, r.AccessType);
		...
		var directUserGroupRules = new Dictionary<Guid, Dictionary<Guid, AccessRule>>();
		...
		foreach (var relation in state.UserGroupRelations)
		{
			...
			var rule = new AccessRule(relation.Id, relation.AccessType);

			if (!directUserGroupRules.TryGetValue(userId, out var userRules))
			{
				userRules = new Dictionary<Guid, AccessRule>();
		...
		foreach (var user in state.Users)
		{
			...
			var globalRule = userGlobalRules.TryGetValue(userGuid, out var userGlobalRule) ? userGlobalRule : AccessRule.GetDefault();

			// Оптимизация: объединение групповых правил
			Dictionary<int, AccessRule> groupSourceRules = null!;
			Dictionary<int, AccessRule> groupBlockRules = null!;
			Dictionary<int, AccessRule> groupTagRules = null!;
			Dictionary<Guid, AccessRule> groupRules = [];
			...
			if (userGroups.TryGetValue(userGuid, out var userGroupSet))
			{
				...
				groupSourceRules = new Dictionary<int, AccessRule>();
				groupBlockRules = new Dictionary<int, AccessRule>();
				groupTagRules = new Dictionary<int, AccessRule>();
			...
			Dictionary<int, AccessRule> userSourceRules = [];
			Dictionary<int, AccessRule> userBlockRules = [];
			Dictionary<int, AccessRule> userTagRules = [];
	...
	private static void AddToMap<TKey, TValue>(
		Dictionary<TKey, Dictionary<TValue, AccessRule>> map,
		TKey owner,
		TValue objId,
		AccessRule rule)
		where TKey : notnull
		where TValue : notnull
	{
		...
		{
			innerMap = new Dictionary<TValue, AccessRule>();
			map[owner] = innerMap;
		}
		innerMap[objId] = rule;
	}
