		...
		IUpdateTagHandler
{
	...
	public override async Task<bool> ExecuteInTransactionAsync(UpdateTagCommand command, CancellationToken ct = default)
	{
		...
		if (command.SourceTagId.HasValue && !await tagsRepository.ExistsAsync(command.SourceTagId.Value, ct))
			throw Errors.NotFoundTag(command.SourceTagId.Value, $"Идентификатор: {nameof(command.SourceTagId)} = {command.SourceTagId}");

		if (command.ThresholdSourceTagId.HasValue && !await tagsRepository.ExistsAsync(command.ThresholdSourceTagId!.Value, ct))
			throw Errors.NotFoundTag(command.ThresholdSourceTagId.Value, $"Идентификатор: {nameof(command.ThresholdSourceTagId)} = {command.SourceTagId}");
	...
	public override InventoryState UpdateCache(InventoryState state)
	{
