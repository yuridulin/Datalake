...
namespace Datalake.InventoryService.Application.Features.UserGroups;

/// <summary>
/// Репозиторий работы с группами пользователей в памяти приложения
/// </summary>
public class UserGroupsMemoryRepository(InventoryCacheStore dataStore)
{
	#region API
	...
	public async Task<UserGroupInfo> CreateAsync(
		InventoryEfContext db, UserAuthInfo user, UserGroupCreateRequest request)
	{
	...
	public async Task<bool> UpdateAsync(
		InventoryEfContext db, UserAuthInfo user, Guid groupGuid, UserGroupUpdateRequest request)
	{
	...
	public async Task<bool> MoveAsync(
		InventoryEfContext db, UserAuthInfo user, Guid groupGuid, Guid? parentGuid)
	{
	...
	public async Task<bool> DeleteAsync(
		InventoryEfContext db, UserAuthInfo user, Guid groupGuid)
	{
		user.ThrowIfNoAccessToUserGroup(AccessType.Manager, groupGuid);
	...
	internal async Task<UserGroupInfo> ProtectedCreateAsync(
		InventoryEfContext db, Guid userGuid, UserGroupCreateRequest request)
	{
		// Проверки, не требующие стейта
		UserGroupEntity newUserGroup = new()
		{
			Guid = Guid.NewGuid(),
		...
		AccessRuleEntity directRuleForUserGroup = new()
		{
	...
	internal async Task<bool> ProtectedUpdateAsync(
		InventoryEfContext db, Guid userGuid, Guid groupGuid, UserGroupUpdateRequest request)
	{
		// Проверки, не требующие стейта
		UserGroupEntity updatedUserGroup;
		AccessRuleEntity? updatedAccessRights = null;
		...
		using (await dataStore.AcquireWriteLockAsync())
		{
			...
			var newUsersRelations = request.Users
				.Select(u => new UserGroupRelationEntity
				{
					UserGuid = u.Guid,
	...
	internal async Task<bool> ProtectedMoveAsync(
		InventoryEfContext db, Guid userGuid, Guid guid, Guid? parentGuid)
	{
		// Проверки, не требующие стейта
		UserGroupEntity movedUserGroup;
	...
	internal async Task<bool> ProtectedDeleteAsync(
		InventoryEfContext db, Guid userGuid, Guid groupGuid)
	{
		// Проверки, не требующие стейта
		UserGroupEntity updatedUserGroup;
	...
	private static async Task LogAsync(InventoryEfContext db, Guid authorGuid, Guid guid, string message, string? details = null)
	{
