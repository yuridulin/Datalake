...
public class InventoryStore : IInventoryStore
{
	...
	private readonly ILogger<InventoryStore> _logger;
	...
	public async Task UpdateAsync(Func<IInventoryState, IInventoryState> update)
	{
		var newState = update(_currentState);
		await _updateChannel.Writer.WriteAsync((InventoryCacheState)newState);
	}

	public IInventoryState State => _currentState;

	public event EventHandler<IInventoryState>? StateChanged;
