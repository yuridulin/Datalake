...
public record class UserEntity : IWithGuidKey, ISoftDeletable
{
	...
	public static UserEntity CreateFromStaticOptions(string name, string token, string? host)
	{
		return new UserEntity
		{
			...
			FullName = name,
			StaticHost = host,
			PasswordHash = PasswordHash.FromExistingHash(token),
		};
	}

	public static UserEntity CreateFromLoginPassword(string login, string password)
	{
		return new UserEntity
		{
			Guid = Guid.NewGuid(),
			Type = UserType.Local,
			Login = login,
			PasswordHash = PasswordHash.FromPlainText(password),
		};
	}
