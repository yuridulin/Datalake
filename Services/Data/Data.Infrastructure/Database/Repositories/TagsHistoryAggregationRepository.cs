...
[Scoped]
public class TagsHistoryAggregationRepository(DataLinqToDbContext db) : ITagsHistoryAggregationRepository
{
	public async Task<TagHistoryAggregationWeightedValue[]> GetWeightedAggregatedValuesAsync(
		int[] identifiers,
		...
		TagResolution period = TagResolution.Hour)
	{
		...
		var result =
			...
			select new TagHistoryAggregationWeightedValue(
				g.Key,
				periodEnd,
				g.Sum(x => x.Weight) ?? 0,
