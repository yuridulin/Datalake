...
namespace Datalake.Data.Infrastructure.Database.QueriesServices;

[Scoped]
public class SourcesQueriesService(DataDbContext context) : ISourcesQueriesService
{
	public async Task<SourceSettingsDto?> GetByIdAsync(int sourceId, CancellationToken cancellationToken)
	{
		return await Query().FirstOrDefaultAsync(x => x.SourceId == sourceId, cancellationToken);
	}
	...
	private IQueryable<SourceSettingsDto> Query()
	{
