	...
	public class Driver : IDriver
	{
		...
		void CreatePackages()
		{
			...
			foreach (var field in Configuration.Fields)
			{
				...
				if (field.Address.Contains("0x"))
				{
					field.AddressDec = ushort.Parse(field.Address.Replace("0x", ""), System.Globalization.NumberStyles.HexNumber);
				}
				else
				{
					field.AddressDec = ushort.TryParse(field.Address, out ushort u) ? u : ushort.MinValue;
				}
			}
			...
			{
				Configuration.Fields = Configuration.Fields.Where(x => x.IsActive).OrderBy(x => x.AddressDec).ToList();
				...
				while (Configuration.Fields.Count(x => !x.Checked) > 0)
				{
					...
					Package package = new Package
					{
						...
						StartAddress = fields[0].AddressDec,
					...
					{
						if (fields[k].AddressDec == fields[k - 1].AddressDec + length)
						{
			...
			{
				foreach (Field field in Configuration.Fields.Where(x => x.IsActive).Where(x => !x.Checked))
				{
					Packages.Add(new Package
					{
						Transaction = i,
						SlaveId = Configuration.SlaveId,
						StartAddress = field.AddressDec,
						OldByteFirst = Configuration.OldByteFirst,
						OldRegisterFirst = Configuration.OldRegisterFirst,
