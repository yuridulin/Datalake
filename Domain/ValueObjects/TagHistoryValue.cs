...
public sealed record class TagHistoryValue
{
	...
	public static TagHistoryValue AsEmpty(int tagId, DateTime? date, TagQuality? quality)
	{
		return new()
		{
			TagId = tagId,
	...
	public static TagHistoryValue FromRaw(int tagId, TagType type, DateTime? date, TagQuality? quality, object? value, float? scale)
	{
		if (value == null)
	...
	/// <returns>Значение тега</returns>
	public static TagHistoryValue AsString(int tagId, DateTime? date, TagQuality? quality, string? text)
	{
	...
	/// <param name="scale">Шкала преобразования для числового типа</param>
	/// <returns>Значение тега</returns>
	public static TagHistoryValue AsNumeric(int tagId, DateTime? date, TagQuality? quality, float? number, float? scale)
	{
		var value = AsEmpty(tagId, date, quality);
	...
	/// <returns>Значение тега</returns>
	public static TagHistoryValue AsBoolean(int tagId, DateTime? date, TagQuality? quality, bool? boolean)
	{
		var value = AsEmpty(tagId, date, quality);
	...
	/// Новым будет считаться, если не старее по времени и содержит новые данные
	/// </summary>
	public bool IsNew(TagHistoryValue newValue)
	{
