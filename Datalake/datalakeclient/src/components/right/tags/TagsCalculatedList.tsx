...
import { CalculatedId } from "../../../@types/enums/CustomSourcesIdentity";
...
export default function TagsCalculatedList() {
	...
	function load() {
		axios.get(API.tags.getCalculatedTags)
			.then(res => setTags(res.data))
	}

	function createManual() {
		axios.post(API.tags.create, { sourceId: CalculatedId })
			.then(res => {
				if (res.data.Done) {
					load()
					setUpdate(new Date())
	...
	return <>
		...
		>Список вычисляемых тегов</Header>
		<FormRow title="Поиск">
